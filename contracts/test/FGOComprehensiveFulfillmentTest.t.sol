...
contract FGOComprehensiveFulfillmentTest is Test {
    ...
    function setUp() public {
        ...
        templateChild1 = new FGOTemplateChild(
            7,
            INFRA_ID,
            address(accessControl),
            address(supplyCoordination),
        ...
        templateChild2 = new FGOTemplateChild(
            8,
            INFRA_ID,
            address(accessControl),
            address(supplyCoordination),
    ...
    function testComprehensiveFulfillmentWithComplexPayments() public {
        ...
        FGOLibrary.ChildReference[]
            memory template1Refs = new FGOLibrary.ChildReference[](2);
        ...
        assertEq(
            ...
            supplier1Initial +
                expectedDirectChild1Payment +
                expectedNestedChild1Payment,
            "Supplier1 should receive direct + nested child1 payments"
        );
        assertEq(
            mona.balanceOf(supplier2),
            supplier2Initial + expectedNestedChild2Payment,
            "Supplier2 should receive nested child2 payment"
        );
        assertEq(
            mona.balanceOf(supplier3),
            supplier3Initial + expectedTemplate1Payment,
            "Supplier3 should receive template payment"
        );

        assertEq(parent.balanceOf(buyer1), 1, "Buyer should own parent token");
        assertEq(
            child1.balanceOf(buyer1, baseChild1),
            0,
            "Child tokens should be reserved for fulfillment"
        );
        assertEq(
            templateChild1.balanceOf(buyer1, template1),
            0,
            "Template tokens should be reserved for fulfillment"
        );
        ...
        fulfillment.completeStep(
            orderId,
            0,
            "Design finalized, materials sourced"
        );
        vm.stopPrank();
        ...
        fulfillment.completeStep(
            orderId,
            1,
            "Manufacturing complete with specialist input"
        );
        vm.stopPrank();
        ...
        assertEq(
            child1.balanceOf(buyer1, baseChild1),
            0,
            "Tokens should not mint until all physical steps complete"
        );
        ...
        fulfillment.completeStep(
            orderId,
            2,
            "Quality control passed, ready to ship"
        );
        vm.stopPrank();
        ...
        assertEq(
            child1.balanceOf(buyer1, baseChild1),
            3,
            "Buyer should receive all direct + nested child1 tokens"
        );
        assertEq(
            child2.balanceOf(buyer1, baseChild2),
            1,
            "Buyer should receive nested child2 tokens from template"
        );
        assertEq(
            templateChild1.balanceOf(buyer1, template1),
            1,
            "Buyer should receive template token"
        );

        FGOLibrary.PhysicalRights memory child1Rights = child1
            .getPhysicalRights(baseChild1, orderId, buyer1, address(market));
        FGOLibrary.PhysicalRights memory child2Rights = child2
            .getPhysicalRights(baseChild2, orderId, buyer1, address(market));
        FGOLibrary.PhysicalRights memory templateRights = templateChild1
            .getPhysicalRights(template1, orderId, buyer1, address(market));

        assertEq(
            child1Rights.guaranteedAmount,
            0,
            "All child1 rights should be consumed"
        );
        assertEq(
            child2Rights.guaranteedAmount,
            0,
            "All child2 rights should be consumed"
        );
        assertEq(
            templateRights.guaranteedAmount,
            0,
            "All template rights should be consumed"
        );
    }
    ...
    function testDigitalOnlyFulfillmentCompletion() public {
        ...
        FGOLibrary.ChildReference[]
            memory digitalRefs = new FGOLibrary.ChildReference[](1);
        ...
        uint256 digitalParentId = parent.reserveParent(
            FGOLibrary.CreateParentParams({
        ...
        assertEq(
            mona.balanceOf(supplier1),
            supplier1Initial + 5 ether,
            "Supplier should receive child payment"
        );

        assertEq(parent.balanceOf(buyer1), 1, "Buyer should own parent token");
        assertEq(
            child1.balanceOf(buyer1, digitalChild),
            1,
            "Digital tokens should be minted immediately"
        );
        ...
        fulfillment.completeStep(
            orderId,
            0,
            "Digital asset created and optimized"
        );
        vm.stopPrank();
        ...
        assertEq(
            child1.balanceOf(buyer1, digitalChild),
            1,
            "Digital tokens remain with buyer during fulfillment"
        );
        ...
        fulfillment.completeStep(
            orderId,
            1,
            "Digital validation complete, ready for delivery"
        );
        vm.stopPrank();
        ...
        assertEq(
            child1.balanceOf(buyer1, digitalChild),
            1,
            "Digital tokens remain with buyer after completion"
        );
    }

    function testComplexPaymentSplitsWithSubPerformers() public {
        vm.startPrank(supplier1);
        uint256 baseChild = child1.createChild(
            FGOLibrary.CreateChildParams({
        ...
        FGOLibrary.ChildReference[]
            memory splitRefs = new FGOLibrary.ChildReference[](1);
        ...
        uint256 splitParentId = parent.reserveParent(
            FGOLibrary.CreateParentParams({
                digitalPrice: 0,
        ...
        fulfillment.completeStep(
            orderId,
            0,
            "Manufacturing complete with specialist contributions"
        );
        vm.stopPrank();

        assertEq(
            child1.balanceOf(buyer1, baseChild),
            1,
            "Buyer should receive child token after completion"
        );

        uint256 totalExpectedPayment = 100 ether + 20 ether;
        uint256 totalActualPayment = (mona.balanceOf(designer1) -
            designer1Initial) +
        ...
        assertEq(
            ...
            "Total payments should equal expected amount"
        );
    }

    function testMixedDigitalPhysicalBehavior() public {
        vm.startPrank(supplier1);
        uint256 mixedChild = child1.createChild(
            FGOLibrary.CreateChildParams({
        ...
        FGOLibrary.ChildReference[]
            memory mixedRefs = new FGOLibrary.ChildReference[](1);
        ...
        uint256 mixedParentId = parent.reserveParent(
            FGOLibrary.CreateParentParams({
        ...
        FGOMarketLibrary.PurchaseParams[]
            memory digitalParams = new FGOMarketLibrary.PurchaseParams[](1);
        ...
        FGOMarketLibrary.PurchaseParams[]
            memory physicalParams = new FGOMarketLibrary.PurchaseParams[](1);
        ...
        assertEq(
            ...
            2,
            "Buyer should own both digital and physical parent tokens"
        );
        ...
        fulfillment.completeStep(
            digitalOrderId,
            0,
            "Digital fulfillment complete"
        );
        vm.stopPrank();

        assertEq(
            child1.balanceOf(buyer1, mixedChild),
            1,
            "Buyer should receive digital child token"
        );
        ...
        assertEq(
            child1.balanceOf(buyer1, mixedChild),
            2,
            "Buyer should receive second child token from physical"
        );
        ...
        assertEq(
            ...
            0,
            "Physical rights should be consumed from physical purchase"
        );
    }
}
