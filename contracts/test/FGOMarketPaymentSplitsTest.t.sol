...
contract FGOMarketPaymentSplitsTest is Test {
    ...
    function testComplexFulfillmentPaymentSplits() public {
        ...
        FGOLibrary.SubPerformer[]
            memory digitalSubs = new FGOLibrary.SubPerformer[](2);
        ...
        FGOLibrary.SubPerformer[]
            memory physicalSubs = new FGOLibrary.SubPerformer[](1);
        ...
        uint256 expectedDesignerPayment = (expectedParentPayment * 9000) / 10000;
        ...
        uint256 totalExpectedPayment = expectedChildPayment +
            expectedParentPayment;
        assertEq(
            mona.balanceOf(buyer1),
            buyer1Initial - totalExpectedPayment,
            "Buyer should pay total amount (full parent + child price)"
        );
        ...
        assertEq(
            ...
            0,
            "Child tokens reserved for fulfillment, not minted yet"
        );
    ...
    function testDeepNestedTemplatePayments() public {
        ...
        vm.startPrank(supplier1);
        FGOLibrary.ChildReference[]
            memory level2Placements = new FGOLibrary.ChildReference[](1);
        ...
        assertEq(
            mona.balanceOf(supplier1),
            supplier1Initial +
                expectedTemplatePayment +
                expectedBaseChildPayment,
            "Supplier should receive both template and nested child payments"
        );

        // Verify deep nested minting
        assertEq(
            templateChild.balanceOf(buyer1, level2Template),
            1,
            "Buyer should own level 2 template"
        );
        assertEq(
            child1.balanceOf(buyer1, baseChild),
            1,
            "Buyer should own nested base child"
        );
    }
}
