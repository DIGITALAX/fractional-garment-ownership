...
contract FGOChildFunctionalityTest is Test {
    ...
    function testChildUpdate() public {
        ...
        FGOLibrary.CreateChildParams memory createParams = FGOLibrary.CreateChildParams({
            digitalPrice: 100,
            physicalPrice: 200,
            version: 1,
            maxPhysicalFulfillments: 1000,
        ...
        uint256 childId = childContract.createChild(createParams);
    ...
    function testMarketApprovalWorkflow() public {
        ...
        FGOLibrary.ChildMarketApprovalRequest memory request = childContract.getMarketRequest(childId, market1);
    ...
    function testParentApprovalWorkflow() public {
        ...
        childContract.requestParentApproval(childId, 1);
        
        FGOLibrary.ParentApprovalRequest memory request = childContract.getParentRequest(childId, 1, parent1);
        assertTrue(request.isPending);
        ...
        vm.startPrank(supplier1);
        childContract.approveParentRequest(childId, 1, parent1);
    ...
    function testTemplateApprovalWorkflow() public {
        ...
        childContract.requestTemplateApproval(childId, 1);
        
        FGOLibrary.TemplateApprovalRequest memory request = childContract.getTemplateRequest(childId, 1, template1);
        assertTrue(request.isPending);
        ...
        vm.startPrank(supplier1);
        childContract.approveTemplateRequest(childId, 1, template1);
    ...
    function testDirectApprovalMethods() public {
        ...
        childContract.approveParent(childId, 1, parent1);
        assertTrue(childContract.approvesParent(childId, 1, parent1));
        
        childContract.approveMarket(childId, market1);
        assertTrue(childContract.approvesMarket(childId, market1));
        
        childContract.approveTemplate(childId, 1, template1);
        assertTrue(childContract.approvesTemplate(childId, 1, template1));
    ...
    function testBatchChildCreation() public {
        ...
        FGOLibrary.CreateChildParams[] memory batchParams = new FGOLibrary.CreateChildParams[](3);
        ...
        for (uint256 i = 0; i < 3; i++) {
            batchParams[i] = FGOLibrary.CreateChildParams({
                ...
                physicalReferencesOpenToAll: false,
                preferredPayoutCurrency: paymentToken,
                childUri: string(abi.encodePacked("ipfs://child", vm.toString(i))),
                authorizedMarkets: markets
            });
        }
        
        uint256[] memory childIds = childContract.createChildrenBatch(batchParams);
        ...
        for (uint256 i = 0; i < 3; i++) {
            FGOLibrary.ChildMetadata memory metadata = childContract.getChildMetadata(childIds[i]);
            assertEq(metadata.digitalPrice, 100 + i);
            assertEq(metadata.physicalPrice, 200 + i);
            assertEq(metadata.supplier, supplier1);
        }
    ...
    function testInvalidChildOperations() public {
        ...
        vm.expectRevert(FGOErrors.AddressInvalid.selector);
        childContract.approveParent(999, 1, parent1);
