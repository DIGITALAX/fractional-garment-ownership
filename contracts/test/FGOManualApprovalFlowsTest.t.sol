...
contract FGOManualApprovalFlowsTest is Test {
    ...
    function testManualTemplateApprovalFlow() public {
        ...
        assertFalse(templateChild.isChildActive(templateId), "Template should not auto-activate");

        // Step 2: Manually approve template for child
        child1.approveTemplateRequest(childId, templateId, 2, address(templateChild));

        // Step 3: Create template - should now succeed
        templateChild.createTemplate(templateId);
        
        // Template should now be ACTIVE
        assertTrue(templateChild.isChildActive(templateId), "Template should be active after manual approval and creation");
    ...
    function testManualTemplateRejectionFlow() public {
        ...
        child1.rejectTemplateRequest(childId, templateId, address(templateChild));
        ...
        assertFalse(templateChild.isChildActive(templateId), "Template should remain inactive after rejection");
    ...
    function testApprovalStateToggling() public {
        ...
        child1.approveTemplateRequest(childId, templateId, 3, address(templateChild));
        
        // Should be able to create now
        templateChild.createTemplate(templateId);
        assertTrue(templateChild.isChildActive(templateId), "Template should be active after approval");
        ...
        child1.approveTemplateRequest(childId, templateId2, 3, address(templateChild));
        
        // Creation should work now
        templateChild.createTemplate(templateId2);
        assertTrue(templateChild.isChildActive(templateId2), "Template2 should be active after initial approval");
        ...
        child1.approveTemplateRequest(childId, templateId3, 3, address(templateChild));
        
        // Creation should work
        templateChild.createTemplate(templateId3);
        assertTrue(templateChild.isChildActive(templateId3), "Template3 should be active after approval");
    ...
    function testManualParentApprovalFlow() public {
        ...
        assertTrue(parent.designExists(parentId), "Parent should remain active after creation");
    ...
    function testManualParentRejectionFlow() public {
        ...
        vm.startPrank(supplier1);
        child1.rejectParentRequest(childId, parentId, address(parent));
        vm.stopPrank();
    ...
    function testComplexManualNestedApprovals() public {
        ...
        FGOLibrary.ChildReference[] memory level1Placements = new FGOLibrary.ChildReference[](2);
        ...
        assertFalse(templateChild.isChildActive(level1Template), "Level 1 template should not auto-activate");

        // Step 1: Approve level 1 template for both base children
        child1.approveTemplateRequest(baseChild1, level1Template, 2, address(templateChild));
        
        vm.stopPrank();
        vm.startPrank(supplier2); // Different supplier owns child2
        child2.approveTemplateRequest(baseChild2, level1Template, 1, address(templateChild));
        vm.stopPrank();
        vm.startPrank(supplier1);

        // Now create level 1 template
        templateChild.createTemplate(level1Template);
        assertTrue(templateChild.isChildActive(level1Template), "Level 1 template should be active");
        ...
        uint256 baseChild3 = child3.createChild(FGOLibrary.CreateChildParams({
        ...
        FGOLibrary.ChildReference[] memory level2Placements = new FGOLibrary.ChildReference[](2);
        ...
        assertFalse(templateChild.isChildActive(level2Template), "Level 2 template should not auto-activate");
        ...
        vm.stopPrank();
        vm.startPrank(supplier1);
        templateChild.approveTemplateRequest(level1Template, level2Template, 1, address(templateChild));
        // Also need baseChild1 to approve level2Template since it's nested through level1Template
        child1.approveTemplateRequest(baseChild1, level2Template, 2, address(templateChild));
        vm.stopPrank();

        // Need supplier2 to approve baseChild2 for level2Template (nested through level1Template)
        vm.startPrank(supplier2);
        child2.approveTemplateRequest(baseChild2, level2Template, 1, address(templateChild));
        vm.stopPrank();

        // Need supplier3 to approve child3 for level2 template  
        vm.startPrank(supplier3);
        child3.approveTemplateRequest(baseChild3, level2Template, 3, address(templateChild));

        // Now create level 2 template
        templateChild.createTemplate(level2Template);
        assertTrue(templateChild.isChildActive(level2Template), "Level 2 template should be active after all approvals");
        ...
        vm.startPrank(supplier3);
        templateChild.approveParentRequest(level2Template, parentId, 2, address(parent));
        // baseChild3 approval (also owned by supplier3, nested in level2Template)
        child3.approveParentRequest(baseChild3, parentId, 6, address(parent)); // 2 * 3 = 6 total amount needed
        vm.stopPrank();

        // level1Template approval (supplier1 owns level1Template, nested in level2Template)  
        vm.startPrank(supplier1);
        templateChild.approveParentRequest(level1Template, parentId, 2, address(parent)); 
        // baseChild1 approval (also owned by supplier1, nested in level1Template nested in level2Template)
        child1.approveParentRequest(baseChild1, parentId, 4, address(parent)); // 2 * 2 = 4 total amount needed
        vm.stopPrank();

        // baseChild2 approval (supplier2 owns baseChild2, nested in level1Template nested in level2Template)
        vm.startPrank(supplier2);
        child2.approveParentRequest(baseChild2, parentId, 2, address(parent)); // 2 * 1 = 2 total amount needed
        vm.stopPrank();
        ...
        assertTrue(parent.designExists(parentId), "Complex nested parent should be active after all manual approvals");
    ...
    function testMixedApprovalAndRejectionFlow() public {
        ...
        uint256 child1Id = child1.createChild(FGOLibrary.CreateChildParams({
            digitalPrice: 1 ether,
            physicalPrice: 2 ether,
            version: 1,
            maxPhysicalEditions: 100,
        ...
        uint256 child2Id = child2.createChild(FGOLibrary.CreateChildParams({
            digitalPrice: 1.5 ether,
            physicalPrice: 2.5 ether,
            version: 1,
            maxPhysicalEditions: 100,
        ...
        uint256 child3Id = child3.createChild(FGOLibrary.CreateChildParams({
            digitalPrice: 2 ether,
            physicalPrice: 3 ether,
            version: 1,
            maxPhysicalEditions: 100,
        ...
        vm.startPrank(supplier1);
        child1.approveTemplateRequest(child1Id, templateId, 1, address(templateChild));
        vm.stopPrank();

        vm.startPrank(supplier2);
        child2.rejectTemplateRequest(child2Id, templateId, address(templateChild));
        vm.stopPrank();

        vm.startPrank(supplier3);
        child3.approveTemplateRequest(child3Id, templateId, 1, address(templateChild));
        vm.stopPrank();
    ...
    function testApprovalRevocationScenario() public {
        ...
        child1.approveTemplateRequest(childId, template1Id, 1, address(templateChild));
        templateChild.createTemplate(template1Id);
        assertTrue(templateChild.isChildActive(template1Id), "Template 1 should be active");
