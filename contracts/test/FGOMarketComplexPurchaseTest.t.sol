...
contract FGOMarketComplexPurchaseTest is Test {
    ...
    function testParentPurchaseWithManyNestedChildrenAndTemplates() public {
        ...
        uint256 directChild1 = child1.createChild(FGOLibrary.CreateChildParams({
        ...
        uint256 templateChild1 = child2.createChild(FGOLibrary.CreateChildParams({
        ...
        uint256 templateChild2 = child1.createChild(FGOLibrary.CreateChildParams({
        ...
        assertEq(mona.balanceOf(designer1), designer1Balance + expectedParentPayment, "Designer should receive parent payment");
        assertEq(mona.balanceOf(supplier1), supplier1Balance + expectedDirectChildPayment, "Supplier1 should receive direct child payment only");
        assertEq(mona.balanceOf(supplier2), supplier2Balance + expectedNestedChild1Payment + expectedNestedChild2Payment, "Supplier2 should receive both nested child payments");
        assertEq(mona.balanceOf(supplier3), supplier3Balance + expectedTemplatePayment, "Template supplier should receive template payment");
        
        // Verify minting
        assertEq(parent.balanceOf(buyer1), 1, "Buyer should own 1 parent token");
        assertEq(child1.balanceOf(buyer1, directChild1), 1, "Buyer should own 1 direct child1 from parent");
        assertEq(templateChild.balanceOf(buyer1, level1Template), 1, "Buyer should own 1 template from parent");
        assertEq(child2.balanceOf(buyer1, templateChild1), 3, "Buyer should own 3 nested child1 from template");
        assertEq(child1.balanceOf(buyer1, templateChild2), 2, "Buyer should own 2 nested child2 from template");
    }
    ...
    function testBatchPurchaseMultipleItems() public {
        ...
        assertEq(mona.balanceOf(supplier1), supplier1Balance + expectedChild1Payment + expectedChild2DirectPayment + expectedChild2NestedPayment, "Supplier1 should receive all payments");
        assertEq(mona.balanceOf(supplier2), supplier2Balance + expectedTemplatePayment, "Supplier2 should receive template payment");
        
        // Verify minting
        assertEq(child1.balanceOf(buyer1, child1Id), 2, "Buyer should own 2 child1 tokens");
        assertEq(child2.balanceOf(buyer1, child2Id), 3, "Buyer should own 3 child2 tokens (1 direct + 2 from template)");
