...
contract FGOMarketAvailabilityTest is Test {
    ...
    function testMixedAvailabilityPurchases() public {
        ...
        uint256 physicalOnlyChild = child2.createChild(FGOLibrary.CreateChildParams({
        ...
        assertEq(mona.balanceOf(supplier1), supplier1InitialBalance + expectedTemplatePayment, "Template supplier should receive payment");
        assertEq(mona.balanceOf(supplier2), supplier2InitialBalance + expectedPhysicalOnlyPayment, "Physical-only supplier should receive payment");
        assertEq(mona.balanceOf(supplier3), supplier3InitialBalance + expectedBothChildrenPayment, "Both-availability supplier should receive payment");
        
        // Verify minting - only physical-available items minted
        assertEq(templateChild.balanceOf(buyer2, mixedTemplate), 1, "Buyer2 should own 1 template token");
        assertEq(child1.balanceOf(buyer2, digitalOnlyChild), 0, "Buyer2 should NOT own digital-only child from physical purchase");
        assertEq(child2.balanceOf(buyer2, physicalOnlyChild), 1, "Buyer2 should own 1 physical-only child");
        assertEq(child3.balanceOf(buyer2, bothChild), 2, "Buyer2 should own 2 both-availability children");
    ...
    function testEdgeCaseMaxPhysicalEditions() public {
        ...
        FGOMarketLibrary.PurchaseParams[] memory params1 = new FGOMarketLibrary.PurchaseParams[](1);
        ...
        assertEq(child1.balanceOf(buyer1, limitedChild), 2, "Buyer1 should own 2 child tokens");
        
        // Second physical purchase should fail (no more physical editions available)
        vm.startPrank(buyer2);
        FGOMarketLibrary.PurchaseParams[] memory params2 = new FGOMarketLibrary.PurchaseParams[](1);
        ...
        vm.startPrank(buyer2);
        FGOMarketLibrary.PurchaseParams[] memory params3 = new FGOMarketLibrary.PurchaseParams[](1);
        ...
        assertEq(child1.balanceOf(buyer2, limitedChild), 5, "Buyer2 should own 5 digital child tokens");
    }
}