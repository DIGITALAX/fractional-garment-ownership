...
contract FGOMarketBasicPurchaseTest is Test {
    ...
    function testSingleChildDigitalPurchase() public {
        ...
        assertEq(
            ...
            supplier1InitialBalance + expectedPayment,
            "Supplier should receive payment"
        );
        assertEq(
            mona.balanceOf(buyer1),
            buyer1InitialBalance - expectedPayment,
            "Buyer should pay for child"
        );

        // Verify minting
        assertEq(
            child1.balanceOf(buyer1, childId),
            2,
            "Buyer should own 2 child tokens"
        );

        FGOLibrary.ChildMetadata memory childMeta = child1.getChildMetadata(
            childId
        );
        assertEq(
            childMeta.currentPhysicalEditions,
            0,
            "Physical editions should remain 0 for digital purchase"
        );
    }
    ...
    function testSingleChildPhysicalPurchase() public {
        ...
        FGOLibrary.ChildMetadata memory childMetaBefore = child1
        ...
        assertEq(
            child1.balanceOf(buyer1, childId),
            3,
            "Buyer should own 3 child tokens"
        );

        FGOLibrary.ChildMetadata memory childMetaAfter = child1
            .getChildMetadata(childId);
        assertEq(
            childMetaAfter.currentPhysicalEditions,
            childMetaBefore.currentPhysicalEditions + 3,
            "Physical editions should increase by 3"
        );
    ...
    function testSingleTemplateWithNestedChildrenPurchase() public {
        ...
        assertEq(
            mona.balanceOf(supplier1),
            supplier1InitialBalance + expectedChild1Payment,
            "Supplier1 should receive child1 payment"
        );
        assertEq(
            mona.balanceOf(supplier2),
            supplier2InitialBalance + expectedChild2Payment,
            "Supplier2 should receive child2 payment"
        );
        assertEq(
            mona.balanceOf(supplier3),
            supplier3InitialBalance + expectedTemplatePayment,
            "Supplier3 should receive template payment"
        );

        uint256 totalExpectedPayment = expectedTemplatePayment +
            expectedChild1Payment +
            expectedChild2Payment;
        assertEq(
            mona.balanceOf(buyer1),
            buyer1InitialBalance - totalExpectedPayment,
        ...
        assertEq(
            ...
            1,
            "Buyer should own 1 child2 token"
        );
        assertEq(
